 ...
 *  at new array points
 * 
 *  Example is hard-coded for a specific s3d data set
 */
...
int main(int argc, char **argv) {
    ...
    // For testing purposes: if we set fileOrderC=True, then "spoof" a C-ordering
    // by changing the shape of the file.
    if (fileOrderC)
    {
        shape = {550, 540, 704};
    }
    ...
    // Test conditions
    if (world.rank() == 0 && dom_dim == 2 && nblocks[0]*nblocks[1]*nblocks[2] == 6) {
        Block<real_t, int> *b = static_cast<Block<real_t, int>*>(master.block(0));
        int expectedBlockMax = (fileOrderC ? 3 : 4);
        int blockMax = (int) b->max_errs_reduce[1];
        real_t max_red_err = b->max_errs_reduce[0];
        if (blockMax != expectedBlockMax || fabs(max_red_err - 1.0896873215513452) > 1.e-10) {
        ...
        //
        if (world.size() == 1 && !fileOrderC ) {
            Block<real_t, int> *bmax = static_cast<Block<real_t, int>*>(master.block(
                    blockMax));
            MatrixX<real_t> &bl = bmax->blend->domain;
            //std::cout<<bl(0,2) << "\n";
            if  ( (fabs(bl(0, 3) - 31.079045650447384) > 1.e-7 ) ||
                  (fabs(bl(1, 3) - 31.175520176541) > 1.e-7 ) )
            {
                std::cout << std::setprecision(14)
                          <<  "expected blend(0,3) = 31.07904565044  got "  << bl(0, 3) << "\n";
                std::cout <<  "expected blend(1,3) = 31.175520176541  got "  << bl(1, 3) << "\n";
                abort();
            }
            else
            {
                std::cout << std::setprecision(14) <<  "passed test blend(0, 3) == "
                        << bl(0, 3) << "  blend(1, 3) == " << bl(1, 3)<< "\n";
            }
        }
        //std::cout << bmax->blend << "\n";
    }
    if (world.rank() == 0 && dom_dim == 3 && nblocks[0]*nblocks[1]*nblocks[2] == 6) {
        Block<real_t, int> *b = static_cast<Block<real_t, int>*>(master.block(0));
        int blockMax = (int) b->max_errs_reduce[1];
        real_t max_red_err = b->max_errs_reduce[0];
