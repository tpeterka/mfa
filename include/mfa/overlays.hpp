...
namespace mfa
{
    ...
    struct Tmesh
    {
        ...
        // n-d knots in index space mapped to vector of tensors in increasing depth containing the knot
        // knot -> [increasing-depth tensors containing the knot]
        unordered_map<KnotIdx, vector<TensorIdx>>           knot_tensors;

        // 1-d knots in each dimension in index space mapped to vector of tensors in increasing depth containing the knot
        // knot[dim] -> [increasing-depth tensors containing the knot]
        ...
        // update 1-d knot->tensors hash maps for all tensors
        // the same knot can appear in multiple tensors stored in a vector in increasing depth order
            ...
            for (auto j = 0; j < tensor_prods.size(); j++)                                      // for all tensors
            {
                for (auto i = 0; i < dom_dim_; i++)                                             // for all dims
                {
                    for (auto k = 0; k < tensor_prods[j].knot_idxs[i].size(); k++)              // for all knots in the current dim
                    {
                        ...
                        t_idxs.push_back(j);
                        knot_dim_tensors[i][k_idx] = t_idxs;                                    // hash the knot to the tensor
                    }
                }
            }
        }

        // update n-d knot->tensors hash map for all tensors
        // the same knot can appear in multiple tensors stored in a vector in increasing depth order
            ...
            for (auto j = 0; j < tensor_prods.size(); j++)              // for all tensors
            {
                ...
                while (!tensor_knots_iter.done())                            // for all knots in the tensor
                {
                    ...
                    if (!skip)
                    {
                        ...
                        vector<KnotIdx> t_idxs;
                        if (knot_tensors.find(idx) != knot_tensors.end())
                            t_idxs = knot_tensors.at(idx);
                        t_idxs.push_back(j);
                        knot_tensors[idx] = t_idxs;          // hash the knot to the tensor
                    }
        ...
        // fills vector of tensors in increasing depth containing n-d knot in index space
...
//             fmt::print(stderr, "anchors(): target[0] {}\n", target[0]);
                ...
                vector<vector<KnotIdx>>&        anchor_extents) const       // (output) possibly expanded first and last anchors in all dims
        {
            ...
            for (auto a = 0; a < orig_anchors[0].size(); a++)               // for all multidim original anchors
            {
                ...
                // if the anchor doesn't exist in any tensor, this is the indicator to expand the anchors
                if (!lookup_tensor(anchor, found_tidxs))
                {
                    // debug
//                     fmt::print(stderr, "expand_anchors() 0: anchor [{}] not found, anchor_extents[0] [{}] anchor_extents [1] [{}]\n",
//                             fmt::join(anchor, ","), fmt::join(anchor_extents[0], ","), fmt::join(anchor_extents[1], ","));

                    // TODO: should expand symmetrically only the dimension where the knot is missing, but
                    // since we don't know which dimension is the problem, expand symmetrically in all dims
                    for (auto i = 0; i < dom_dim_; i++)
                    {
                        if (anchor_extents[i][0] > 0)
                            anchor_extents[i][0]--;
                        if (anchor_extents[i][1] < all_knots[i].size() - 1)
                            anchor_extents[i][1]++;
                    }

                    // debug
//                     fmt::print(stderr, "expand_anchors() 1: anchor [{}] not found, anchor_extents[0] [{}] anchor_extents [1] [{}]\n",
//                             fmt::join(anchor, ","), fmt::join(anchor_extents[0], ","), fmt::join(anchor_extents[1], ","));
                }
            }
        ...
        // read control points from a text file and replace them in a TensorProduct
        // generated by OpenCode
        void ReadCtrlPtsFromFile(
                ...
                TensorProduct<T>&       tensor)
        {
            ...
            // Read the file into a vector of points
            ...
            // Validate the data
            if (points.empty()) {
                cerr << "Error: No valid control points found in file" << endl;
                return;
            }

            // Check dimensionality consistency
            size_t pt_dim = points[0].size();
            for (const auto& point : points) {
                if (point.size() != pt_dim) {
                    cerr << "Error: Inconsistent point dimensions in file" << endl;
                    return;
                }
            }

            // Check if the number of points matches the tensor's expectation
            size_t expected_points = tensor.nctrl_pts.prod();
            fmt::print(stderr, "ReadCtrlPtsFromFile(): filename {} num ctrl pts = {}\n", filename, expected_points);
            if (points.size() != expected_points) {
                cerr << "Warning: File contains " << points.size() << " points, "
                    << "but tensor expects " << expected_points << " points" << endl;

                // Option 1: Resize the tensor's control points matrix
                tensor.ctrl_pts.resize(points.size(), pt_dim);

                // Option 2: Only use the first expected_points
                // points.resize(expected_points);
            }

            // Create Eigen matrix from the points
            MatrixX<T> new_ctrl_pts(points.size(), pt_dim);
            for (size_t i = 0; i < points.size(); ++i) {
                for (size_t j = 0; j < pt_dim; ++j) {
                    new_ctrl_pts(i, j) = points[i][j];
                }
            }

            // Replace the control points in the tensor
            tensor.ctrl_pts = new_ctrl_pts;

            // Optional: Reset weights to 1.0 if needed
            if (tensor.weights.size() != tensor.ctrl_pts.rows()) {
                tensor.weights = VectorX<T>::Ones(tensor.ctrl_pts.rows());
            }
        }
    };
}
