...
//                     fprintf(stderr, "Using VolPt\n");
                ...
                VolPt_tmesh(param, mfa_data.tmesh.tensor_prods[0].nctrl_pts, mfa_data.tmesh.tensor_prods[0].ctrl_pts,
                        mfa_data.tmesh.tensor_prods[0].weights, cpt);
...
//                     fprintf(stderr, "Using VolPt\n");
                         ...
                         VectorX<T>&            out_pt)     // (output) point, allocated by caller
        {
            ...
            vector<vector<size_t>> anchors(mfa_data.dom_dim);                        // anchors affecting the decoding point
...
//             fprintf(stderr, "\n--------------------------\n\n");
            ...
            vector<size_t> center(mfa_data.dom_dim);                         // center anchor from anchors, offset by (p+1)/2
            for (auto i = 0; i < mfa_data.dom_dim; i++)
            {
...
//                 cerr << "N[" << i << "]: " << N[i].row(0) << endl;
            ...
            VolPt(param, out_pt, mfa_data.tmesh.tensor_prods[0], unused, N, nctrl_pts, ctrl_pts, weights);
...
//             cerr << "out_pt: " << out_pt.transpose() << "\n" << endl;
                                                ...
                                                // pass size-0 vector if unused
        {
            ...
            vector<int>         n(mfa_data.p.size());              // number of control point spans in each dim
